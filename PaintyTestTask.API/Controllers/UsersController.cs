...
namespace PaintyTestTask.API.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        [HttpPut("add")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> Add(User user) => Ok(await _repository.Add(user));

        [HttpDelete("delete")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> Delete(User user) => Ok(await _repository.Delete(user));
        
        [HttpDelete("delete/id/{id:guid}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> DeleteById(Guid id) => Ok(await _repository.DeleteById(id));
